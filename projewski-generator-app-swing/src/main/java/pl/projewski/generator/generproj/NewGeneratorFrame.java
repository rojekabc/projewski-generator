...
*/		getContentPane().add(scrPanParam);
	...
	public NewGeneratorFrame( GeneratorInterface gi, String name,
			CreationInterface ci, String setParam )
	{
		this( gi, name, ci );
		if ( setParam != null )
		{
...
*/		Object [] objs = GenerProj.listGeneratorTypes();
		for ( int i=0; i<objs.length; i++ )
			comType.addItem( objs[i] );	
		if ( gi != null )
		{
			for ( int i=0; i<comType.getItemCount(); i++ )
			{
				ClassItem classItem = (ClassItem) comType.getItemAt(i);
...
/*	protected void setActualGenerator( GeneratorInterface gi, String name )
	...
	{
		try {
			ClassItem sel = (ClassItem) comType.getSelectedItem();
			if ( sel != null )
			{
				if ( (inGI == null ) || (inGI.getClass() != sel.getClassItem() ) )
				{
					Object o = sel.getClassItem().newInstance();
	...
	private GeneratorInterface buildGenerator()
	{
		try {
			...
			if ( sel == null )
				return null;
			GeneratorInterface gi = (GeneratorInterface) sel.getClassItem().newInstance();
			String params [] = gi.listParameters();
			for ( int i=0; i<params.length; i++)
			{
				Object value = paramEl.getParameterValue(i);
				if ( value != null )
					gi.setParameter( params[i], value );
				else
				{
					JOptionPane.showMessageDialog( this,
							Convert.stringWrap(
							"Niepoprawnie wprowadzony parametr \""+gi.listParameters()[i]+"\"",30),
			...
			return gi;
		} catch ( Exception e ) { // TODO: Lepsze wyłapywanie
			e.printStackTrace();
		}
		return null;
	}
	...
	private boolean saveGenerator()
	{
			...
			Object sel = comType.getSelectedItem();
			if ( sel == null )
			{
				JOptionPane.showMessageDialog( this,
		...
		else if ( o == butType )
		{
			...
			try {
				if ( d.getSelectedFile() == null ) // nacisnięcie Rezygnuj
					return;
				ClassItem newItem = GenerProj.addGeneratorType(
						d.getSelectedFile().getPath() );
				if ( newItem != null )
					comType.addItem( newItem );
...
*/			}
		...
		else if ( o == buttDesc )
		{
			...
			if ( classItem == null )
				return;
			String className = classItem.getClassItem().getName();
			
			if ( GenerProj.isHelpFile( className ) )
				GenerProj.showHelp( className );
			else { // Try show help from description from ParameterInterface
				try {
					GeneratorInterface gi = (GeneratorInterface) classItem.getClassItem().newInstance();
